...
// function to detect keypressing in linux
// no input, output is 1 or 0 depending on if the key is pressed
int kbhit()
{
...
// no input, output is updated coordinate of the bullets in the map
void Bullet::Move()
{
    this->x += this->V_x;
    this->y += this->V_y;
...
// input is map size and difficulty level, output is where the bullets will spawn (randomized) and how big it is (based on difficulty level)
...
// no input, output is getting bullet position in the vertical axis
...
// no input, output is getting bullet position in the horizontal axis
...
// no input, output is getting bullet size
...
// no input, output is getting x coordinate of the player
...
// no input, output is getting y coordinate of the player
...
// input is map size, output is player coordinate is updated from keypressing
...
// no input, output is the player's starting position is randomized, and starting health is 3
void Player::init()
{
    this->x = rand()%11 + 5;
    this->y = rand()%21 + 10;
...
// input is mapsize, output is randomized coordinate of healing items
...
// no input, output is fixed map size in x coordinate and y coordinate
void Game::initMap()
{
    this->MapSize_x = 20;
    this->MapSize_y = 40;
...
// no input, output is initialization of game over, player, clearance of bullets and heal items, and maximum bullets in the map
...
// constructor
// no input, output is initialization of map and variables
Game::Game()
{
    this->initMap();
    this->initVariables();
...
// ----------------------------------------------FUNCTIONS INVOLVING GAME RUNNING--------------------------------------------------------------- //

// if !gameOver is true, stop the game
// no input, output is boolean returning false if game is over
bool Game::isRunning()
{
    return !gameOver;
}

// -----------------------------------------------FUNCTIONS INVOLVING UNDATING STATUS----------------------------------------------------------- //

// function to update the map constantly
// no input, output is the map that is constantly printed, randomized bullet movements in the map
// minus 1 health if the player is detected to hit the bullet, time of invicibility for player after getting hit
// update the player position in the map, and spawn healing items and delete them randomly
void Game::updateMap()
{
    // resetting the map
...
// no input, output is move function is initialized
...
// no input, output is to move the bullets, and if it is out of the map, delete it
// and creates bullets by chance
...
// no input, output is healing items are spawned randomly in the map
...
// no input, output is all the update functions mentioned are initialized
void Game::update()
{
    updateBulletStatus();
    spawnHealingItems();
...
// ----------------------------------------------------FUNCTIONS INVOLVING RENDERING---------------------------------------------------------------------- //

// function to render UI
// no input, output is health printed in the map
void Game::renderUI()
{
...
// no input, output is the map printed, and print "You've been hit!!" if player hits bullet
void Game::renderMap()
{
    for(int i = 0; i < this->MapSize_x; i++)
    {
...
// no input, output is all render functions mentioned will be initialized
void Game::render()
{
    renderUI();
    renderMap();
...
// --------------------------------------------FUNCTIONS INVOLVING MENU, DIFFICULTY, SCOREBOARD------------------------------------------------------------ //

// function to print menu
// no input, output is menu is printed in the start of the game
void Game::printMenu()
{
...
// no input, output is difficulty level is updated
...
// no input, output is print the highest score of easy, medium, and hard mode
...
// no input, output is player's score is stored in a text file
